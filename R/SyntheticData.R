...
GenerateSynthetic <- function(numberOfSamples,numberOfGenes,status = NULL,cnvProb = 1/10,label = "Sample",lambdaNor = 1000,lamdaAmp = 1.5 * lambdaNor,lambdaDel = 1/1.5 * lambdaNor,rdist = rpois,seed = 123)
{
  ...
  samples = foreach(i = 1:numberOfSamples,.combine=cbind) %:% 
      foreach(j = 1:numberOfGenes,.combine =  c) %do%
   {
     if(is.null(status)) {
        ...
        lambda = sample(c(lambdaDel, lambdaNor, lambdaAmp),1,prob = c(cnvProb,1,cnvProb))
...
testSamples =GenerateSynthetic(numberOfTestSamples,numberOfGenes, cnvProb = 1/10, status = status)
