...
a background noise distribution is estimated. The background reported is
defined by $5\%$ and the $95\%$ of the respective distribution. For each unique
...
\end{figure}
  ...
  \begin{table}[h]
    ...
    distribution\\ \hline
    5\% | 95\% Noise & The quantiles of the background noise.
    Background noise is the distribution of median ratios between
    reference and sample of interest, when no significant change
    is present. \\ \hline
