...
The BED file is required to obtain amplicon and gene names information associated with the panel.
...
Two sets of data are required. The sample of interest and the set of reference bam files to compare against.
...
The reads were counted using the \textbf{ExomeCopy} package from the Bioconductor project. All reads overlapping with the region of an amplicon were counted for this amplicon. Only reads with a mapping quality $\geq 20$ were counted. The function implemented in the ExomeCopy package allows to remove PCR Duplicates. For reads with the same start site, end site and chromosomal orientation only one is kept. PCR duplicates do not have a large influence on the ratio between reference and sample, still this might serve as an additional quality control step in a CNV detection pipeline.
...
You can also use synthetic data to test the functions.
The following examples make use of two synthetically generated data sets.
One for the reference and one as a testing set.

<<LoadingSyntheticData, echo = TRUE>>=
data(testSamples)
data(referenceSamples)
sampleReadCounts <- testSamples
referenceReadCounts <- referenceSamples
## Gene names should be same size as row columns 
geneNames <- row.names(referenceSamples)
...
To account for sample and sequencing run specific variations the counts obtained for each sample were normalized using the \textbf{cn.mops} package.
...
\end{figure}
  ...
  \begin{table}[hb]
    ...
    Median Ratio & The median ratio \\ \hline
    Up & Up Regulated \\ \hline
    Down & Down Regulated \\ \hline
    5\% Quantile & The 5\% quantile of the bootstrap distribution\\ \hline
    50\% Quantile & The 50\% quantile of the bootstrap distribution\\ \hline
    95\% Quantile & The 95\% quantile of the bootstrap distribution\\ \hline
    5\% Noise & The 5\% noise \\ \hline
    95\% Noise & The 95\% noise \\ \hline
    Sign. & Boolean value representing if the read counts differences are Significative \\ \hline
    Above Noise & Boolean value representing if the median ratio is above Noise \\ \hline
    Ampl. Number & Number of Amplicons associated with the gene \\ \hline
    Passed & if there is a detected aberration which the value is above noise the Value will be 1, otherwise 0 \\ \hline
    \end{tabular}
...
<<BootstrapPlot, echo=FALSE, fig.align='center', fig.cap='Plot for a single Test sample', out.width='0.7\\linewidth', fig.height=4, fig.width='\\linewidth'>>=
#PlotBootstrapDistributions(bootList, reportTables, outputDirectory)[[sampleIndexToShow]]
PlotBootstrapDistributions(bootList, reportTables)[[sampleIndexToShow]]
@
  ...
  \centering
    ...
    Non Reliable Change & There is some change but is below the noise level \\ \hline
    Reliable Change & The change is above the noise level \\ \hline
    \end{tabular}
