...
respective distribution. To calculate the mean and standard deviation
of ratios the log ratios were used. The upper bound ratio has to be
below the lower noise (deletion) or the lower bound ratio above the upper
noise (amplification) to be considered reliable. The significance level can
also be passed as parameter.
