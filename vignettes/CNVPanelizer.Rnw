...
This aproach is similar to the Random Forest method, which bootstrap the
samples and subsample the features. In our case features would be equivalent to
...
deletion for an upper bound ratio < 1.
...
by the lower noise $medianNoise - qnorm(0.025) * standardDeviationNoise$ and
the upper noise $medianNoise + qnorm(0.975) * standardDeviationNoise$ of the
respective distribution. The ratio has to be below (deletion) or above (amplification)
the noise distribution to be considered reliable.
...
%    distribution\\ \hline
    ...
    AboveNoise & Boolean value representing if mean ratio is
    above noise \\ \hline
