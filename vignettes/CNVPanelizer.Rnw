...
additional quality control step in the CNV detection pipeline.
...
obtained for each sample were normalized using the TMM (Trimmed Mean of M)
from the \textbf{NOISeq} package.
...
This aproach is similar to Random forest method, which bootstrap the samples
and subsample the features. In our case features would be equivalent to
...
replacement (boostrapping) from the set of reference samples. The ratio between
the sample of interest and each randomized reference is calculated for each
gene, using only a randomly selected subset of amplicons. A significant change
is considered as an amplification for a ratio > 1 and a deletion for a
ratio < 1.
...
# To speed up vignette generation...
replicates <- 10
@
...
by lower noise $medianNoise - sensitivityConstant * standardDeviationNoise$
and upper noise $medianNoise + sensitivityConstant * standardDeviationNoise$
of the respective distribution. The ratio has to be below (deletion) or above
(amplification) this noise distribution to be considered reliable.
...
%    distribution\\ \hline
    Lower | Median | Upper Noise & The Lower/Median/Upper background
    noise bounds. \\ \hline
...
@
  ...
  \centering
    ...
    below the noise level for the number of amplicons for this gene\\ \hline
